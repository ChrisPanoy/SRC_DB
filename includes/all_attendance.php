...
$sql = "
    SELECT
        ...
        COALESCE(
            pa.pc_number,
            (SELECT paa.pc_number FROM lab_pc_assignment paa WHERE paa.student_id = st.student_id ORDER BY paa.date_assigned DESC LIMIT 1)
        ) AS pc_number,
        ...
        fac.lab_name     AS lab
    FROM lab_attendance att
    JOIN lab_admission a   ON att.admission_id = a.admission_id
    JOIN lab_students st   ON a.student_id     = st.student_id
    LEFT JOIN lab_section sec     ON a.section_id     = sec.section_id
    LEFT JOIN lab_year_level yl   ON a.year_level_id  = yl.year_id
    LEFT JOIN lab_subject sub     ON a.subject_id     = sub.subject_id
    LEFT JOIN lab_schedule sc     ON att.schedule_id  = sc.schedule_id
    LEFT JOIN lab_facility fac    ON sc.lab_id        = fac.lab_id
    LEFT JOIN lab_pc_assignment pa ON pa.student_id   = st.student_id AND pa.lab_id = fac.lab_id
    $where_sql
