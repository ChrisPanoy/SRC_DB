...
// Fetch profile picture for logged-in teacher (if any) FROM lab_employees table
...
if (isset($_SESSION['teacher_id']) && isset($conn)) {
    ...
    $pstmt = $conn->prepare("SELECT profile_pic, firstname, lastname FROM lab_employees WHERE employee_id = ? LIMIT 1");
    if ($pstmt) {
