...
$baseSql = "
    ...
    JOIN lab_admission ad ON a.admission_id = ad.admission_id
    JOIN lab_students st  ON ad.student_id   = st.student_id
    LEFT JOIN lab_year_level yl ON ad.year_level_id = yl.year_id
    LEFT JOIN lab_section    sec ON ad.section_id    = sec.section_id
    JOIN lab_schedule sc ON a.schedule_id = sc.schedule_id
    LEFT JOIN lab_subject  subj ON sc.subject_id = subj.subject_id
...
<table class="min-w-full divide-y divide-gray-200 rounded-xl overflow-hidden shadow">
    ...
    <tbody class="bg-white divide-y divide-gray-200">
        ...
        <tr class="hover:bg-blue-50 transition">
            ...
            <?php if ($type === 'admin' || $type === 'all'): ?>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <?= htmlspecialchars($row['lab_name'] ?? '-') ?> <!-- Lab Data FROM lab_facility -->
                </td>
            <?php endif; ?>
            <?php if ($type === 'admin' || $type === 'all'): ?>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
