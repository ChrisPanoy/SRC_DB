...
$total_students = $conn->query("SELECT COUNT(*) as cnt FROM lab_students")->fetch_assoc()['cnt'];
$present_today = $conn->query("SELECT COUNT(DISTINCT student_id) as cnt FROM lab_attendance WHERE DATE(scan_time)='$today' AND status='Present'")->fetch_assoc()['cnt'];

// Get sign in/out counts by year level
$signin_counts = [];
$signout_counts = [];
for($i = 1; $i <= 4; $i++) {
    $signin_counts[] = $conn->query("SELECT COUNT(DISTINCT student_id) as cnt FROM lab_attendance a 
        JOIN lab_students s ON a.student_id = s.student_id 
        WHERE s.year_level = $i AND DATE(a.scan_time)='$today' 
        AND a.status='Present'")->fetch_assoc()['cnt'];
    
    $signout_counts[] = $conn->query("SELECT COUNT(DISTINCT student_id) as cnt FROM lab_attendance a 
        JOIN lab_students s ON a.student_id = s.student_id 
        WHERE s.year_level = $i AND DATE(a.scan_time)='$today' 
...
$year_query = $conn->query("
    ...
    FROM lab_students s
    LEFT JOIN lab_attendance a ON s.student_id = a.student_id
