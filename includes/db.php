...
try {
    ...
    if (session_status() !== PHP_SESSION_NONE) {
        ...
        $resAy = $conn->query("SELECT ay_id, ay_name FROM lab_academic_year WHERE status = 'Active' LIMIT 1");
        if ($rowAy = $resAy->fetch_assoc()) {
            $_SESSION['active_ay_id'] = (int)$rowAy['ay_id'];
            $_SESSION['active_ay_name'] = $rowAy['ay_name'];
        }

        $resSem = $conn->query("SELECT semester_id, semester_now FROM lab_semester WHERE status = 'Active' LIMIT 1");
