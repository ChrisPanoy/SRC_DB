...
$stmt = $conn->prepare("
    ...
    FROM lab_students st
    LEFT JOIN lab_admission adm ON st.student_id = adm.student_id 
...
$sections = [];
$sec_res = $conn->query("SELECT section_id, section_name FROM lab_section ORDER BY section_name ASC");
...
$year_levels = [];
$yl_res = $conn->query("SELECT year_id, year_name FROM lab_year_level ORDER BY year_id ASC");
...
$labs = [];
$lab_stmt = $conn->prepare("SELECT lab_id, lab_name FROM lab_facility ORDER BY lab_name ASC");
...
// Helper: load current pc_number for this student + selected lab FROM lab_pc_assignment
$current_pc_number = '';
if ($selected_lab_id > 0) {
    $pc_stmt = $conn->prepare("SELECT pc_number FROM lab_pc_assignment WHERE student_id = ? AND lab_id = ? LIMIT 1");
    if ($pc_stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($section_id > 0 && $year_level_id > 0) {
        $upd_adm = $conn->prepare("UPDATE lab_admission SET section_id = ?, year_level_id = ? WHERE student_id = ? AND academic_year_id = ? AND semester_id = ?");
    ...
    if ($selected_lab_id <= 0) {
        ...
        if ($pc_number !== '') {
            ...
            $check_stmt = $conn->prepare("SELECT student_id FROM lab_pc_assignment WHERE pc_number = ? AND lab_id = ? AND student_id != ? LIMIT 1");
            if ($check_stmt) {
                ...
                if ($check_res && $check_res->num_rows > 0) {
                    ...
                    // Upsert INTO lab_pc_assignment
                    $existing_stmt = $conn->prepare("SELECT pc_assignment_id FROM lab_pc_assignment WHERE student_id = ? AND lab_id = ? LIMIT 1");
                    if ($existing_stmt) {
                        ...
                        if ($existing) {
                            $update_stmt = $conn->prepare("UPDATE lab_pc_assignment SET pc_number = ?, date_assigned = CURDATE() WHERE pc_assignment_id = ?");
                            if ($update_stmt) {
                            ...
                            $insert_stmt = $conn->prepare("INSERT INTO lab_pc_assignment (student_id, lab_id, pc_number, date_assigned) VALUES (?, ?, ?, CURDATE())");
                            if ($insert_stmt) {
            ...
            $delete_stmt = $conn->prepare("DELETE FROM lab_pc_assignment WHERE student_id = ? AND lab_id = ?");
            if ($delete_stmt) {
