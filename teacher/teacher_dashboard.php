...
// Fetch teacher profile picture FROM lab_employees table (teacher_id holds employee_id)
$profile_pic = null;
$profile_stmt = $conn->prepare("SELECT profile_pic FROM lab_employees WHERE employee_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === 0) {
    ...
    $update_pic = $conn->prepare("UPDATE lab_employees SET profile_pic=? WHERE employee_id=?");
...
$subjects_sql = "
    SELECT 
        ...
        (SELECT COUNT(DISTINCT student_id) FROM lab_admission WHERE schedule_id = sc.schedule_id) AS total_students
    FROM lab_schedule sc
    JOIN lab_subject subj   ON sc.subject_id = subj.subject_id
    LEFT JOIN lab_facility fac ON sc.lab_id = fac.lab_id
...
// Load all labs FROM lab_facility so teachers can reassign labs per schedule
$labs_options = [];
if ($labs_res = $conn->query("SELECT lab_id, lab_name FROM lab_facility ORDER BY lab_name")) {
...
$attendance_overview_sql = "
    ...
    JOIN lab_admission adm ON a.admission_id = adm.admission_id
    JOIN lab_schedule sc ON adm.schedule_id = sc.schedule_id
...
$subject_distribution_sql = "
    ...
    FROM lab_schedule sc
    JOIN lab_subject subj ON sc.subject_id = subj.subject_id
    LEFT JOIN lab_admission adm ON adm.schedule_id = sc.schedule_id
...
$present_sql = "
    ...
    JOIN lab_subject subj ON sc.subject_id = subj.subject_id
    LEFT JOIN lab_admission adm ON adm.schedule_id = sc.schedule_id
    LEFT JOIN lab_students st ON adm.student_id = st.student_id
    LEFT JOIN lab_attendance a 
...
$gender_sql = "
    ...
    FROM lab_attendance a
    JOIN lab_admission adm ON a.admission_id = adm.admission_id
    JOIN lab_students st ON adm.student_id = st.student_id
    JOIN lab_schedule sc ON adm.schedule_id = sc.schedule_id
    WHERE a.attendance_date = ?
      AND a.status IN ('Present','Late')
      AND sc.employee_id = ?
    ...
    for ($i = 1; $i <= 2; $i++) {
        ...
        if ($sched_id > 0) {
            ...
            if ($lab_id <= 0) {
                ...
                $lab_res = $conn->query("SELECT lab_id FROM lab_facility LIMIT 1");
            ...
            if ($chk = $conn->query("SHOW COLUMNS FROM lab_schedule LIKE 'days'")) {
                if ($chk->num_rows > 0) $dayCol = 'days';
            }
            $upd = $conn->prepare("UPDATE lab_schedule SET time_start=?, time_end=?, $dayCol=?, lab_id=? WHERE schedule_id=? AND employee_id=?");
            ...
            $ay_id = (int)($_SESSION['active_ay_id'] ?? 0);
            $sem_id = (int)($_SESSION['active_sem_id'] ?? 0);
            $ins = $conn->prepare("INSERT INTO lab_schedule (lab_id, subject_id, employee_id, time_start, time_end, $dayCol, academic_year_id, semester_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
$latest_stmt = $conn->prepare("
    ...
    JOIN lab_schedule sc ON adm.schedule_id = sc.schedule_id
    JOIN lab_subject subj ON sc.subject_id = subj.subject_id
    LEFT JOIN lab_section sec ON adm.section_id = sec.section_id
    LEFT JOIN lab_year_level yl ON adm.year_level_id = yl.year_id
    LEFT JOIN lab_pc_assignment pa ON pa.student_id = st.student_id AND pa.lab_id = sc.lab_id
    WHERE sc.employee_id = ?
                        ...
                        function updateCharts() {
                            ...
                            fetch(url.toString(), { credentials: 'same-origin' })
                                ...
                                .then(data => {
                                    ...
                                    // UPDATE lab_attendance chart (Time In vs Time Out)
