...
function is_timeout_allowed($conn, $schedule_id) {
    // Get schedule info
    $stmt = $conn->prepare("SELECT time_start, time_end FROM lab_schedule WHERE schedule_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$subject_id || !$barcode) {
        ...
        if ($student) {
            ...
            // Here $subject_id is actually schedule.schedule_id
            $regStmt = $conn->prepare("SELECT COUNT(*) as cnt FROM lab_admission WHERE student_id = ? AND schedule_id = ?");
            ...
            if (empty($regCount)) {
                ...
                $admIdStmt = $conn->prepare("SELECT admission_id FROM lab_admission WHERE student_id = ? AND schedule_id = ? LIMIT 1");
                ...
                if (!$admRow) {
                    ...
                    $check = $conn->prepare("SELECT attendance_id, time_in, time_out FROM lab_attendance WHERE admission_id = ? AND schedule_id = ? AND attendance_date = ? LIMIT 1");
                    ...
                    if (!$doWrite) {
                        ...
                        if (!$attRow) {
                            ...
                            $schedStmt = $conn->prepare("SELECT time_start, time_end FROM lab_schedule WHERE schedule_id = ?");
                            ...
                            $ins = $conn->prepare("INSERT INTO lab_attendance (attendance_date, schedule_id, time_in, status, admission_id) VALUES (?, ?, ?, ?, ?)");
                            ...
                            if (empty($attRow['time_out'])) {
                                ...
                                if (!is_timeout_allowed($conn, $subject_id)) {
                                    ...
                                    $upd = $conn->prepare("UPDATE lab_attendance SET time_out = ? WHERE attendance_id = ?");
