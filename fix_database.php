...
try {
    ...
    $columns = $conn->query("SHOW COLUMNS FROM lab_attendance LIKE 'semester_id'");
    if ($columns && $columns->num_rows == 0) {
        echo "Adding semester_id column...<br>";
        $alter = $conn->query("ALTER TABLE `lab_attendance` ADD COLUMN semester_id INT DEFAULT 1 AFTER subject_id");
        if ($alter) {
    ...
    if ($fk_check && $fk_check->num_rows == 0) {
        ...
        $fk = $conn->query("ALTER TABLE `lab_attendance` ADD CONSTRAINT fk_attendance_semester FOREIGN KEY (semester_id) REFERENCES semesters(id) ON UPDATE CASCADE");
        if ($fk) {
    ...
    $update = $conn->query("UPDATE lab_attendance SET semester_id = 1 WHERE semester_id IS NULL OR semester_id = 0");
    if ($update) {
