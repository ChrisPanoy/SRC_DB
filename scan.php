...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $student = $conn->query("SELECT * FROM lab_students WHERE barcode='$barcode'")->fetch_assoc();
    ...
    if ($student) {
        ...
        while ($subj = $subjects_now->fetch_assoc()) {
            ...
            $check_sql = "SELECT id FROM lab_attendance WHERE student_id=? AND subject_id=? AND DATE(scan_time)=? AND status='Present'";
            ...
            if ($check_stmt->num_rows == 0) {
                ...
                $dedupe_check = $conn->prepare("SELECT scan_time FROM lab_attendance WHERE student_id=? AND subject_id=? ORDER BY scan_time DESC LIMIT 1");
                ...
                if ($shouldInsert) {
                    ...
                    $insert_sql = "INSERT INTO lab_attendance (student_id, subject_id, semester_id, scan_time, status) VALUES (?, ?, ?, ?, 'Present')";
        ...
        while ($subj = $subjects_ended->fetch_assoc()) {
            ...
            $check_present_sql = "SELECT id FROM lab_attendance WHERE student_id=? AND subject_id=? AND DATE(scan_time)=? AND status='Present'";
            ...
            $check_signout_sql = "SELECT id FROM lab_attendance WHERE student_id=? AND subject_id=? AND DATE(scan_time)=? AND status='Signed Out'";
            ...
            if ($present_exists && !$signout_exists) {
                ...
                $insert_sql = "INSERT INTO lab_attendance (student_id, subject_id, semester_id, scan_time, status) VALUES (?, ?, ?, ?, 'Signed Out')";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param('siis', $student_id, $subj['id'], 1, $now);
                $insert_stmt->execute();
                $insert_stmt->close();
