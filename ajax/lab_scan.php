...
try {
    ...
    $stuStmt = $conn->prepare('SELECT student_id, name, profile_pic, course, year_level, section FROM lab_students WHERE barcode = ?');
    if (!$stuStmt) {
    ...
    // Dedupe guard
    $dedupe_seconds = 5;
    $dedupe = $conn->prepare('SELECT scan_time, status FROM lab_attendance WHERE student_id = ? AND subject_id = ? ORDER BY scan_time DESC LIMIT 1');
    ...
    $presentChk = $conn->prepare("SELECT COUNT(*) AS c FROM lab_attendance WHERE student_id = ? AND subject_id = ? AND DATE(scan_time) = ? AND status = 'Present'");
    ...
    if ((int)$presentCount === 0) {
        $ins = $conn->prepare("INSERT INTO lab_attendance (student_id, subject_id, subject_name, semester_id, scan_time, status) VALUES (?, ?, ?, ?, ?, 'Present')");
    ...
    $signedOutChk = $conn->prepare("SELECT COUNT(*) AS c FROM lab_attendance WHERE student_id = ? AND subject_id = ? AND DATE(scan_time) = ? AND status = 'Signed Out'");
    ...
    $ins2 = $conn->prepare("INSERT INTO lab_attendance (student_id, subject_id, subject_name, semester_id, scan_time, status) VALUES (?, ?, ?, ?, ?, 'Signed Out')");
