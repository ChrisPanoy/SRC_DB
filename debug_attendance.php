...
$res_st = $conn->query("SELECT student_id, first_name, last_name FROM lab_students WHERE first_name LIKE '%$student_name%' OR last_name LIKE '%$student_name%'");
while ($s = $res_st->fetch_assoc()) {
    ...
    while ($adm = $res_adm->fetch_assoc()) {
        ...
        echo "      Attendance:\n";
        $res_att = $conn->query("SELECT * FROM lab_attendance WHERE admission_id = $aid");
