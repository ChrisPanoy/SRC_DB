...
try {
    ...
    echo "<h2>Step 3: UPDATE lab_attendance Table</h2>";
    $check_attendance_column = $conn->query("SHOW COLUMNS FROM lab_attendance LIKE 'semester_id'");
    if ($check_attendance_column && $check_attendance_column->num_rows == 0) {
        $add_column = $conn->query("ALTER TABLE `lab_attendance` ADD COLUMN semester_id INT DEFAULT 1 AFTER subject_id");
        if ($add_column) {
    ...
    if ($fk_attendance_check && $fk_attendance_check->fetch_assoc()['count'] == 0) {
        $add_fk_attendance = $conn->query("ALTER TABLE `lab_attendance` ADD CONSTRAINT fk_attendance_semester FOREIGN KEY (semester_id) REFERENCES semesters(id) ON UPDATE CASCADE");
    ...
    $update_attendance = $conn->query("UPDATE lab_attendance SET semester_id = 1 WHERE semester_id IS NULL OR semester_id = 0");
