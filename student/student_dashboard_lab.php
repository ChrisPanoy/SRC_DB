...
if ($lab_id_param > 0) {
    ...
    $lab_id = $lab_id_param;
    // Optional: try to resolve a friendly name FROM lab_facility, but don't fail if missing.
    $lab_stmt = $conn->prepare("SELECT lab_name FROM lab_facility WHERE lab_id = ? LIMIT 1");
    if ($lab_stmt) {
    ...
    if ($lab_stmt = $conn->prepare("SELECT lab_id, lab_name FROM lab_facility WHERE lab_name = ? LIMIT 1")) {
    ...
    if ($lab_id === 0) {
        ...
        if ($token !== '') {
            $like = '%' . $token . '%';
            if ($lab_stmt = $conn->prepare("SELECT lab_id, lab_name FROM lab_facility WHERE lab_name LIKE ? ORDER BY lab_id ASC LIMIT 1")) {
...
if ($lab_id_param === 0 && $lab_name_param === '' && $lab_id === 0 && $lab_name === '') {
    $lab_id = 1;
    // Try to resolve a friendly name FROM lab_facility, but fall back if not found
    $lab_stmt = $conn->prepare("SELECT lab_name FROM lab_facility WHERE lab_id = 1 LIMIT 1");
    if ($lab_stmt) {
        $lab_stmt->execute();
        $lab_res = $lab_stmt->get_result();
        if ($lab_row = $lab_res->fetch_assoc()) {
...
if ($lab_id > 0) {
    $activeSql = "SELECT 
                    ...
                    sch.schedule_days,
                    sub.subject_name
                 FROM lab_schedule sch
                 JOIN lab_subject sub ON sch.subject_id = sub.subject_id
                 WHERE sch.lab_id = ?
                   AND (
            ...
            if (!$active) {
                ...
                if (!$admission) {
                    ...
                    if (!$attRow) {
                        ...
                        $insSql = "INSERT INTO lab_attendance (attendance_date, schedule_id, time_in, status, admission_id)
                                   VALUES (?, ?, ?, ?, ?)";
...
if ($lab_id > 0) {
    $feedSql = "SELECT 
                ...
                JOIN lab_students stu       ON adm.student_id = stu.student_id
                JOIN lab_schedule sch       ON a.schedule_id = sch.schedule_id
                JOIN lab_subject sub        ON sch.subject_id = sub.subject_id
                LEFT JOIN lab_pc_assignment pa ON pa.student_id = stu.student_id AND pa.lab_id = sch.lab_id
                WHERE sch.lab_id = ?
