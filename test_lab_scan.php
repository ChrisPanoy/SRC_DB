...
$studentCount = $conn->query("SELECT COUNT(*) as count FROM lab_students")->fetch_assoc()['count'];
...
try {
    $recent = $conn->query("SELECT a.*, s.name FROM lab_attendance a LEFT JOIN lab_students s ON a.student_id = s.student_id ORDER BY a.scan_time DESC LIMIT 5");
