...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    $delete_stmt = $conn->prepare("DELETE FROM lab_subject WHERE subject_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_subject'])) {
    ...
    if (empty($subject_code) || empty($subject_name)) {
        ...
        $check = $conn->prepare("SELECT subject_id FROM lab_subject WHERE subject_code = ? AND subject_id != ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $update_stmt = $conn->prepare("UPDATE lab_subject SET subject_code = ?, subject_name = ? WHERE subject_id = ?");
            ...
            if ($update_stmt->execute()) {
                ...
                if ($teacher_id) {
                    ...
                    $upd_sched = $conn->prepare("UPDATE lab_schedule SET employee_id = ? WHERE subject_id = ? AND academic_year_id = ? AND semester_id = ?");
                    ...
                    if ($upd_sched->affected_rows == 0) {
                        $check_sched = $conn->prepare("SELECT schedule_id FROM lab_schedule WHERE subject_id = ? AND academic_year_id = ? AND semester_id = ?");
                        ...
                        if ($check_sched->get_result()->num_rows == 0) {
                             ...
                             $chkDays = $conn->query("SHOW COLUMNS FROM lab_schedule LIKE 'days'");
                             if ($chkDays && $chkDays->num_rows > 0) $dayCol = 'days';
                             
                             $ins_sched = $conn->prepare("INSERT INTO lab_schedule (lab_id, subject_id, employee_id, time_start, time_end, $dayCol, academic_year_id, semester_id) VALUES (?, ?, ?, '08:00:00', '09:00:00', 'Mon', ?, ?)");
...
$query = "
    ...
    FROM lab_subject s
    LEFT JOIN lab_schedule sc ON sc.subject_id = s.subject_id AND sc.academic_year_id = ? AND sc.semester_id = ?
    LEFT JOIN lab_employees e ON sc.employee_id = e.employee_id AND e.role IN ('Dean','Faculty')
";
...
$teachers = $conn->query("SELECT employee_id, firstname, lastname, role FROM lab_employees WHERE role IN ('Dean','Faculty') ORDER BY lastname, firstname");
?>
