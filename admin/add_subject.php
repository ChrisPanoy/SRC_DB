...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $check = $conn->prepare("SELECT subject_id FROM lab_subject WHERE subject_code = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        // UPDATE lab_subject name and units if they changed
        $lec = (float)($_POST['units_lec'] ?? 0);
        $lab = (float)($_POST['units_lab'] ?? 0);
        $total = $lec + $lab;
        
        $sql = "UPDATE lab_subject SET subject_name = ?, units = ?, lecture = ?, laboratory = ? WHERE subject_id = ?";
        ...
        $sql = "INSERT INTO lab_subject (subject_code, subject_name, units, lecture, laboratory) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
    ...
    if ($new_subject_id && empty($error)) {
        ...
        if (!empty($schedules_to_add)) {
            ...
            $chk = $conn->query("SHOW COLUMNS FROM lab_schedule LIKE 'days'");
            if ($chk && $chk->num_rows > 0) $dayCol = 'days';

            $ay_id  = (int)($_SESSION['active_ay_id'] ?? 0);
            $sem_id = (int)($_SESSION['active_sem_id'] ?? 0);

            $sched_ins = $conn->prepare("INSERT INTO lab_schedule (lab_id, subject_id, employee_id, time_start, time_end, $dayCol, academic_year_id, semester_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
$labs = $conn->query("SELECT lab_id, lab_name FROM lab_facility ORDER BY lab_name");
?>
