...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($email) || empty($name) || empty($password)) {
        ...
        $check = $conn->prepare("SELECT employee_id FROM lab_employees WHERE email = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            try {
                ...
                $stmt = $conn->prepare("INSERT INTO lab_employees (employee_id, firstname, lastname, email, password, role, profile_pic) VALUES (?, ?, ?, ?, ?, 'Faculty', ?)");
