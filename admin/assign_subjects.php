...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assign_subject'])) {
    ...
    if (empty($student_ids) || empty($schedule_ids)) {
        ...
        $checkDup = $conn->prepare("SELECT admission_id FROM lab_admission
                                      WHERE student_id = ? AND subject_id = ? AND schedule_id = ?
                                        AND academic_year_id = ? AND semester_id = ?");
        $checkNull = $conn->prepare("SELECT admission_id FROM lab_admission
                                      WHERE student_id = ? AND academic_year_id = ? AND semester_id = ?
                                        AND subject_id IS NULL AND schedule_id IS NULL LIMIT 1");
        $updFromNull = $conn->prepare("UPDATE lab_admission SET subject_id = ?, schedule_id = ? WHERE admission_id = ?");
        $ins = $conn->prepare("INSERT INTO lab_admission
        ...
        if ($resCourse = $conn->query("SELECT course_id FROM lab_course ORDER BY course_id ASC LIMIT 1")) {
            if ($rowCourse = $resCourse->fetch_assoc()) {
                $cid_int = (int)$rowCourse['course_id'];
            }
        }

        if ($resYl = $conn->query("SELECT year_id FROM lab_year_level ORDER BY year_id ASC LIMIT 1")) {
            if ($rowYl = $resYl->fetch_assoc()) {
                $yl_int = (int)$rowYl['year_id'];
            }
        }

        if ($resSec = $conn->query("SELECT section_id FROM lab_section ORDER BY section_id ASC LIMIT 1")) {
        ...
        if ($selected_year_level !== '') {
            if ($stmtYL = $conn->prepare("SELECT year_id FROM lab_year_level WHERE level = ? OR year_name = ? ORDER BY level LIMIT 1")) {
        ...
        if ($selected_section_name !== '') {
            if ($stmtSEC = $conn->prepare("SELECT section_id FROM lab_section WHERE section_name = ? ORDER BY level, section_name LIMIT 1")) {
        ...
        foreach ($schedule_ids as $sid_int) {
            $stmt = $conn->prepare("SELECT subject_id FROM lab_schedule WHERE schedule_id = ?");
...
$courses = $conn->query("SELECT course_id, course_code, course_name FROM lab_course ORDER BY course_code");
$years   = $conn->query("SELECT year_id, year_name, level FROM lab_year_level ORDER BY level");
$sections = $conn->query("SELECT section_id, section_name, level FROM lab_section ORDER BY level, section_name");
$academic_years = $conn->query("SELECT ay_id, ay_name FROM lab_academic_year ORDER BY ay_id DESC");
$semesters = $conn->query("SELECT semester_id, ay_id, semester_now FROM lab_semester ORDER BY semester_id DESC");

// Get all students using new students schema (first_name, middle_name, last_name)
// We avoid non-existent columns like `name` or `lab_year_level` here
$students = $conn->query("SELECT student_id, first_name, middle_name, last_name FROM lab_students ORDER BY last_name, first_name, student_id");
...
$checkCol = $conn->query("SHOW COLUMNS FROM lab_schedule LIKE 'days'");
...
$schedule_sql = "SELECT sc.schedule_id, sc.time_start, sc.time_end, sc.$dayColumn AS schedule_days,
                 ...
                 FROM lab_schedule sc
                 JOIN lab_facility fac ON sc.lab_id = fac.lab_id
                 JOIN lab_subject sub ON sc.subject_id = sub.subject_id
                 JOIN lab_employees emp ON sc.employee_id = emp.employee_id
...
$subject_list_sql = "
    ...
    FROM lab_subject sub
    LEFT JOIN lab_admission ad ON ad.subject_id = sub.subject_id AND ad.academic_year_id = $ay_id AND ad.semester_id = $sem_id
...
if ($resAy = $conn->query("SELECT ay_id FROM lab_academic_year ORDER BY ay_id DESC LIMIT 1")) {
    if ($rowAy = $resAy->fetch_assoc()) { $latestAyId = (int)$rowAy['ay_id']; }
}
if ($resSem = $conn->query("SELECT semester_id FROM lab_semester ORDER BY semester_id DESC LIMIT 1")) {
...
$student_group_sql = "
    ...
    FROM lab_admission ad
    JOIN lab_students s      ON ad.student_id = s.student_id
    JOIN lab_year_level yl   ON ad.year_level_id = yl.year_id
    JOIN lab_section sec     ON ad.section_id = sec.section_id
