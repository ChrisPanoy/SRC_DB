...
function ensureTeacherBarcodeColumn($conn) {
    ...
    $checkSql = "SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'lab_employees' AND COLUMN_NAME = 'barcode'";
    $res = $conn->query($checkSql);
    if ($res && $res->num_rows === 0) {
        // Create the barcode column on employees
        $conn->query("ALTER TABLE `lab_employees` ADD COLUMN `barcode` VARCHAR(50) NULL AFTER `profile_pic`");
        // Add a unique key to prevent duplicates (ignore error if already exists)
        @($conn->query("ALTER TABLE `lab_employees` ADD UNIQUE KEY `unique_barcode` (`barcode`)"));
        // Backfill existing rows so pages depending on barcode work immediately
        $conn->query("UPDATE `lab_employees` SET `barcode` = `employee_id` WHERE `employee_id` IS NOT NULL AND `employee_id` != ''");
    }
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    $check_subjects = $conn->prepare("SELECT COUNT(*) as count FROM lab_schedule WHERE employee_id = ?");
    ...
    if ($subject_count > 0) {
        ...
        $delete_stmt = $conn->prepare("DELETE FROM lab_employees WHERE employee_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_teacher'])) {
    ...
    if (empty($email) || empty($name)) {
        ...
        $old_email = '';
        $old_email_stmt = $conn->prepare("SELECT email FROM lab_employees WHERE employee_id = ?");
        ...
        if (tableExists($conn, 'users')) {
            ...
            $check = $conn->prepare("SELECT employee_id FROM lab_employees WHERE email = ? AND employee_id != ?");
        ...
        if (empty($error)) {
            ...
            try {
                ...
                if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] == 0) {
                    $cur = $conn->prepare("SELECT profile_pic FROM lab_employees WHERE employee_id = ?");
                ...
                $update_stmt = $conn->prepare("UPDATE lab_employees SET email = ?, profile_pic = ? WHERE employee_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['generate_barcode'])) {
    ...
    // Get teacher information FROM lab_employees table using employee_id
    $teacher_query = $conn->prepare("SELECT employee_id, firstname, lastname FROM lab_employees WHERE employee_id = ?");
    ...
    if ($teacher_result->num_rows > 0) {
        ...
        if (empty($teacher_id)) {
            ...
            $update_barcode = $conn->prepare("UPDATE lab_employees SET barcode = ? WHERE employee_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['reset_teacher_id']) && isset($_POST['reset_new_password'])) {
    ...
    if (!empty($reset_new_password)) {
        ...
        if (strlen($reset_new_password) < 8 || !preg_match('/[^a-zA-Z0-9]/', $reset_new_password)) {
            ...
            $res = $conn->query("SELECT email FROM lab_employees WHERE employee_id = $reset_id");
