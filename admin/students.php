...
$years_list = $conn->query("SELECT year_id, year_name FROM lab_year_level ORDER BY level");
$sections_list = $conn->query("SELECT section_id, section_name, level FROM lab_section WHERE section_name LIKE '%A' OR section_name LIKE '%B' ORDER BY level, section_name");
$course_bsis = $conn->query("SELECT course_id FROM lab_course WHERE course_code = 'BSIS' LIMIT 1")->fetch_assoc();
        ...
        if (!$isCsv) {
            ...
            if (($handle = fopen($tmp, 'r')) !== false) {
                ...
                if (!$hasAll) {
                    ...
                    $checkStmt = $conn->prepare("SELECT student_id FROM lab_students WHERE student_id = ?");
                    $insertStmt = $conn->prepare("INSERT INTO lab_students (student_id, rfid_number, first_name, middle_name, last_name, suffix, gender) VALUES (?, ?, ?, ?, ?, ?, ?)");
                    $updateStmt = $conn->prepare("UPDATE lab_students SET rfid_number = ?, first_name = ?, middle_name = ?, last_name = ?, suffix = ?, gender = ? WHERE student_id = ?");
...
if (isset($_POST['add'])) {
    ...
    if ($student_id === '' || $rfid_number === '' || $first_name === '' || $last_name === '' || $gender === '' || $year_level_id === 0 || $section_id === 0) {
        ...
        $check = $conn->prepare("SELECT 1 FROM lab_students WHERE student_id = ? OR rfid_number = ?");
        ...
        if ($check->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO lab_students (student_id, rfid_number, profile_picture, first_name, middle_name, last_name, suffix, gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            if ($stmt->execute()) {
                ...
                if ($ay_id && $sem_id) {
                    $insAdm = $conn->prepare("INSERT INTO lab_admission (student_id, academic_year_id, semester_id, section_id, year_level_id, course_id) VALUES (?, ?, ?, ?, ?, ?)");
...
if (isset($_POST['update'])) {
    ...
    if ($student_id === '' || $rfid_number === '' || $first_name === '' || $last_name === '' || $gender === '') {
        ...
        $check = $conn->prepare("SELECT 1 FROM lab_students WHERE (student_id = ? OR rfid_number = ?) AND student_id != ?");
        ...
        if ($check->num_rows > 0) {
            ...
            if ($profile_picture) {
                $stmt = $conn->prepare("UPDATE lab_students SET student_id = ?, rfid_number = ?, profile_picture = ?, first_name = ?, middle_name = ?, last_name = ?, suffix = ?, gender = ? WHERE student_id = ?");
                ...
                $stmt = $conn->prepare("UPDATE lab_students SET student_id = ?, rfid_number = ?, first_name = ?, middle_name = ?, last_name = ?, suffix = ?, gender = ? WHERE student_id = ?");
            ...
            if ($stmt->execute()) {
                ...
                if ($ay_id && $sem_id && $year_level_id > 0 && $section_id > 0) {
                    ...
                    $updAdm = $conn->prepare("UPDATE lab_admission SET section_id = ?, year_level_id = ? WHERE student_id = ? AND academic_year_id = ? AND semester_id = ?");
                    ...
                    if ($updAdm->affected_rows == 0) {
                        $chk = $conn->prepare("SELECT 1 FROM lab_admission WHERE student_id = ? AND academic_year_id = ? AND semester_id = ? LIMIT 1");
...
if (isset($_GET['delete'])) {
    ...
    if ($delete_sid !== '') {
        $stmt = $conn->prepare("DELETE FROM lab_students WHERE student_id = ?");
...
if (isset($_GET['edit'])) {
    ...
    if ($edit_sid !== '') {
        $stmt = $conn->prepare("SELECT s.*, 
                                     (SELECT year_level_id FROM lab_admission WHERE student_id = s.student_id AND academic_year_id = ? AND semester_id = ? LIMIT 1) as cur_year,
                                     (SELECT section_id FROM lab_admission WHERE student_id = s.student_id AND academic_year_id = ? AND semester_id = ? LIMIT 1) as cur_section
                                FROM lab_students s 
                                WHERE s.student_id = ?");
...
$students_page = $conn->query("SELECT * FROM lab_students ORDER BY last_name, first_name, student_id LIMIT $per_page OFFSET $offset");
?>
...
<script>
<?php
$students = $conn->query("SELECT * FROM lab_students");
