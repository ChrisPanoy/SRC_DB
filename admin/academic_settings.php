...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['set_active_ay'])) {
        ...
        $conn->query("UPDATE lab_academic_year SET status = 'Inactive'");
        $stmt = $conn->prepare("UPDATE lab_academic_year SET status = 'Active' WHERE ay_id = ?");
    ...
    if (isset($_POST['set_active_sem'])) {
        ...
        $conn->query("UPDATE lab_semester SET status = 'Inactive'");
        $stmt = $conn->prepare("UPDATE lab_semester SET status = 'Active' WHERE semester_id = ?");
        $stmt->bind_param("i", $sem_id);
        if ($stmt->execute()) {
            $message = "Semester updated successfully!";
        } else {
            $error = "Failed to UPDATE lab_semester.";
        }
    }
    ...
    if (isset($_POST['add_ay'])) {
        ...
        if (!empty($ay_name)) {
            $stmt = $conn->prepare("INSERT INTO lab_academic_year (ay_name, status) VALUES (?, 'Inactive')");
    ...
    if (isset($_POST['add_semester'])) {
        ...
        if (!empty($semester_now) && $ay_id > 0) {
            $stmt = $conn->prepare("INSERT INTO lab_semester (semester_now, ay_id, status) VALUES (?, ?, 'Inactive')");
...
$academic_years = $conn->query("SELECT * FROM lab_academic_year ORDER BY ay_id DESC");
$semesters = $conn->query("SELECT s.*, ay.ay_name FROM lab_semester s JOIN lab_academic_year ay ON s.ay_id = ay.ay_id ORDER BY s.semester_id DESC");
?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 flex flex-col h-full">
                ...
                <div class="p-6 flex-grow">
                    ...
                    <div class="space-y-4">
                        <?php 
                        $academic_years = $conn->query("SELECT * FROM lab_academic_year ORDER BY ay_name DESC");
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100 flex flex-col h-full">
                ...
                <div class="p-6 flex-grow">
                    ...
                    <div class="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                        <?php 
                        $sem_res = $conn->query("SELECT s.*, ay.ay_name FROM lab_semester s JOIN lab_academic_year ay ON s.ay_id = ay.ay_id ORDER BY ay.ay_name DESC, s.semester_now DESC");
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="ay_id" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-sky-500 outline-none transition-all">
                        <?php 
                        $ay_list = $conn->query("SELECT * FROM lab_academic_year ORDER BY ay_name DESC");
