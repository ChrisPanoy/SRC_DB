...
$courseIds = [];
$courseRes = $conn->query("SELECT course_id FROM lab_course WHERE course_code = 'BSIS' OR course_name LIKE 'Bachelor of Science in Information System%'");
if ($courseRes) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_year'])) {
    ...
    if ($yearToDelete !== '') {
        $delSql = "DELETE a FROM lab_admission a\n"
               . "JOIN lab_year_level yl ON a.year_level_id = yl.year_id\n"
...
$sql = "SELECT DISTINCT st.student_id, yl.year_name, sct.section_name,
        ...
        FROM lab_admission a
        JOIN lab_students st    ON a.student_id     = st.student_id
        JOIN lab_year_level yl  ON a.year_level_id  = yl.year_id
        JOIN lab_section sct    ON a.section_id     = sct.section_id
        JOIN lab_course c       ON a.course_id      = c.course_id
