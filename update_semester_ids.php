...
echo "<h1>UPDATE lab_semester IDs Script</h1>";
...
try {
    ...
    $result = $conn->query("SHOW COLUMNS FROM lab_attendance LIKE 'semester_id'");
    if ($result->num_rows == 0) {
        $conn->query("ALTER TABLE `lab_attendance` ADD COLUMN semester_id INT DEFAULT 1");
    ...
    $update_result = $conn->query("UPDATE lab_attendance SET semester_id = 1 WHERE semester_id IS NULL OR semester_id = 0");
    if ($update_result) {
